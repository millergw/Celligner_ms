...
load_data <- function(data_dir, 
                      ...
                      cclf_file = 'CCLF_mat_only_full_characterization.csv', 
                      annotation_file = NULL, #'Celligner_info.csv',
                      hgnc_file = "hgnc_complete_set_7.24.2018.txt") {
  hgnc.complete.set <- data.table::fread(file.path(data_dir, hgnc_file)) %>% as.data.frame()
  
  TCGA_mat <- data.table::fread(file.path(data_dir, tumor_file)) %>%
    as.data.frame() %>%
  ...
  CCLE_mat <- data.table::fread(file.path(data_dir, cell_line_file)) %>%
    as.data.frame() %>%
  ...
  # TODO: work to get nicer names for the CCLF data.
  CCLF_mat <- data.table::fread(file.path(data_dir, cclf_file)) %>%
    as.data.frame() %>%
    tibble::column_to_rownames('V1') %>%
    as.matrix()
  # CCLF_mat <-  readr::read_csv(file.path(data_dir, cclf_file)) %>% 
  #   as.data.frame() %>%
  #   tibble::column_to_rownames('X1') %>%
  #   as.matrix()
  
  # Add the CCLF data to the CCLE matrix since we want to lump all cell lines together
  CCLE_mat <- rbind(CCLE_mat, CCLF_mat)
...
run_Celligner <- function(data_dir, results_dir, task_name = "", save_intermediate_results = F) {
  ...
  if (save_intermediate_results){
    saveRDS(DE_genes, file = file.path(results_dir, paste(task_name, "DE_genes.rds", sep = "_")))
    saveRDS(TCGA_obj, file = file.path(results_dir, paste(task_name, "TCGA_obj.rds", sep = "_")))
    saveRDS(CCLE_obj, file = file.path(results_dir, paste(task_name, "CCLE_obj.rds", sep = "_")))
  }
  
  print("Computing cPCA...")
  cov_diff_eig <- run_cPCA(TCGA_obj, CCLE_obj, global$fast_cPCA)
  if (save_intermediate_results){
    saveRDS(cov_diff_eig, file = file.path(data_dir, "cov_diff_eig.rds"))
  }
  ...
  print("Regressing out the cPCs that are most variable in the tumor samples...")
  ...
  if (save_intermediate_results){
    saveRDS(mnn_res, file = file.path(results_dir, paste(task_name, "mnn_res.rds", sep = "_")))
  }
  
  print("Combining and clustering the aligned data...")
  ...
  if (save_intermediate_results){
    saveRDS(cov_diff_eig, file = file.path(data_dir, "cov_diff_eig.rds"))
    saveRDS(comb_obj, file = file.path(results_dir, paste(task_name, "comb_obj.rds", sep = "_")))
  }
  # if (save_intermediate_results){
  #   saveRDS(cov_diff_eig, file = file.path(data_dir, "cov_diff_eig.rds"))
  #   saveRDS(mnn_res, file = file.path(results_dir, paste(task_name, "mnn_res.rds", sep = "_")))
  ...
  print("Returning the Seurat object for the combined, aligned data...")
  return(comb_obj) 
}

# Function to save down all the important information after running Celligner
save_celligner_result <- function(final_seurat_obj, results_dir, task_name = "") {
  saveRDS(final_seurat_obj, file = file.path(results_dir, paste(task_name, "final_seurat_obj.rds", sep = "_")))
}

## Executing tasks ##
# # Running with fastPCA = 10 to save time. Running on full set of RNAseq data
# result <- run_Celligner(data_dir = "celligner_runs/plus_cclf_rnaseq_20201005", 
#                         results_dir = "celligner_runs/plus_cclf_rnaseq_20201005/results",
#                         task_name = "only_full_characterization",
#                         save_intermediate_results = T)
# save_celligner_result(result, results_dir = "celligner_runs/plus_cclf_rnaseq_20201005/results",
#                                  task_name = "only_full_characterization")


full_result <- run_Celligner(data_dir = "celligner_runs/plus_cclf_rnaseq_20201005", 
                        results_dir = "celligner_runs/plus_cclf_rnaseq_20201005/results",
                        task_name = "only_full_characterization_v2",
                        save_intermediate_results = T)
